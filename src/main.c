...
enum Errors {NOERR, SDLERR, MALLOCFAIL, NOLETTER};
...
int CreateAndCheckSpace(SDL_Window **win, SDL_Renderer **rend, int h, int w)
{
	...
	if (NULL ==
		(*win = SDL_CreateWindow(TITLE, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, h, w, SDL_WINDOW_SHOWN)))
	{
		printf("Ошибка SDL: %s", SDL_GetError());
		SDL_Quit();
		return SDLERR;
	}
	if (NULL == (*rend = SDL_CreateRenderer(*win, -1, SDL_RENDERER_ACCELERATED)))
	{
		printf("Ошибка SDL: %s", SDL_GetError());
		SDL_DestroyWindow(*win);
		SDL_Quit();
...
int DrawBackground(SDL_Renderer *rend, Uint8 r, Uint8 g, Uint8 b)
{
	if (SDL_SetRenderDrawColor(rend, r, g, b, 0xFF) || SDL_RenderClear(rend))
	{
		return SDLERR;
	}
	return NOERR;
...
int DrawBackRects(SDL_Renderer *rend, SDL_Rect **rectArr, int count, Uint8 r, Uint8 g, Uint8 b, int size, int spacing)
{
	*rectArr = (SDL_Rect *)SDL_malloc(sizeof(SDL_Rect) * count);
	...
	if (SDL_SetRenderDrawColor(rend, r, g, b, 0xFF) || SDL_RenderFillRects(rend, *rectArr, count))
	{
		return SDLERR;
	}
	/*else*/
...
int DrawLetterC(SDL_Renderer *rend, const SDL_Rect *corner, int segment, Uint8 r, Uint8 g, Uint8 b)
{
	SDL_Rect rect = {corner->x + segment, corner->y + segment, segment * 4, segment * 3};

	if (SDL_SetRenderDrawColor(rend, r, g, b, 0xFF) || SDL_RenderFillRect(rend, &rect))
	{
		return SDLERR;
	}
	/*else*/
...
int DrawLetterE(SDL_Renderer *rend, const SDL_Rect *corner, int segment, Uint8 r, Uint8 g, Uint8 b)
{
	SDL_Rect rect1 = {corner->x + segment, corner->y + segment, segment * 3, segment};
	SDL_Rect rect2 = {corner->x + (segment * 2), corner->y + segment * 3, segment * 3, segment};

	if (SDL_SetRenderDrawColor(rend, r, g, b, 0xFF) || SDL_RenderFillRect(rend, &rect1))
	{
		return SDLERR;
	}
...
int DrawLetterM(SDL_Renderer *rend, const SDL_Rect *corner, int segment, Uint8 r, Uint8 g, Uint8 b)
{
	SDL_Rect rect1 = {corner->x + segment, corner->y + segment, segment, segment * 4};
	SDL_Rect rect2 = {corner->x + segment * 3, corner->y + segment, segment, segment * 4};
...
int DrawLetterH(SDL_Renderer *rend, const SDL_Rect *corner, int segment, Uint8 r, Uint8 g, Uint8 b)
{
	SDL_Rect rect1 = {corner->x + segment * 2, corner->y, segment, segment * 2};
	SDL_Rect rect2 = {corner->x + segment * 2, corner->y + segment * 3, segment, segment * 2};
